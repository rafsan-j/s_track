...
export default function ManageCurriculumPage() {
  ...
  async function fetchCurriculum(courseId: string) {
    const [modulesRes, topicsRes] = await Promise.all([
      ...
      // To get topics, we'll fetch all topics, but we'll filter them by the modules we have in the UI later
  ...
  async function handleAddModule(e: React.FormEvent) {
    ...
    const { error } = await supabase.from("modules").insert({
      user_id: userId, course_id: selectedCourseId, title: moduleTitle
    });
    if (!error) {
      setModuleTitle("");
  ...
  async function handleAddTopic(e: React.FormEvent) {
    ...
    const { error } = await supabase.from("topics").insert({
      user_id: userId, module_id: selectedModuleIdForTopic, title: topicTitle
    });
    if (!error) {
      setTopicTitle("");
      fetchCurriculum(selectedCourseId);
